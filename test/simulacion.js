...
const els={}; for(const id of ['spm','chrono','quality','qtext','diag','warn','btnMain','btnChrono','btnReset']) els[id]=mk();
let now=0; const L={};
...
let fails=0;
async function run({label, mount, spm, axisIdx, seconds=40, hz=60, noise=8, drift=25, gen}){
  ...
  const ok = spm===null ? shown==='--' : (err<=3);
  if(!ok) fails++;
  console.log((ok?'  ok  ':' FALLO'), label.padEnd(30), 'real='+String(spm===null?'--':spm).padStart(4),
              'medido='+shown.padStart(4), '|', q.padEnd(30), '|', d);
}

(async()=>{
  console.log('--- MONTAJE CHALECO (movil vertical, rotacion de tronco) ---');
  for(const spm of [50,60,72,85,100,120,140]) await run({label:'chaleco '+spm+' spm', mount:'chaleco', spm, axisIdx:1});
